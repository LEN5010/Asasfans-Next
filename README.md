...
| 模块 | 已有源码 | 主要缺口 |
| --- | --- | --- |
| 内容与首页 | 视频主流（全部/切片/录播）、二创、历史动态、小说目录与阅读；按来源支持搜索/筛选/随机、自动续页、图文与大图查看 | 视频源无标题搜索；小说尚无收藏/阅读进度/本地规则；真实来源与布局待验收 |
...
当前已接入中性明暗主题、材质偏好、自适应导航、共享卡片/弹层和四端原 A 图标；主体 UI/组件改造已落地，当前按人类反馈修复。SDK 升级已完成，[历史四端开发构建](https://github.com/LEN5010/Asasfans-Next/actions/runs/35874899297)在 `3b14066` 成功，不覆盖后续迭代，界面和设备效果由用户验收，不用历史测试数量代替。

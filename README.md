...
当前采用中性明暗主题和统一玻璃交互控件；正文卡保持实色。首页依次为日历、历史上的今天、最新切片、最新二创，历史大卡手动横滑。小说默认全部，R18 仅提供元信息与原帖入口。五个 UI/UX 开发阶段已落地，源码、编译与用户验收分别记录。SDK 升级已完成，[历史四端开发构建](https://github.com/LEN5010/Asasfans-Next/actions/runs/35874899297)在 `3b14066` 成功，不覆盖后续迭代，界面和设备效果由用户验收，不用历史测试数量代替。
